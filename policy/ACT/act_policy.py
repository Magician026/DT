...
def require_deploy_artifacts(directory):
    if not directory:
        raise ValueError("ACT deployment requires ckpt_dir")
...
class ACT:
    ...
    def __init__(self, args_override=None, RoboTwin_Config=None):
        ...
        require_deploy_artifacts(args_override.get("ckpt_dir"))
        ...
        if ckpt_dir:
            ...
            if os.path.exists(ckpt_path):
                loading_status = self.policy.load_state_dict(torch.load(ckpt_path, map_location="cpu", weights_only=True))
