...
def test_backward_reaches_static_dynamic_gate_and_shared_backbone():
    ...
    embedding, detail_dynamic = encoder.forward_with_details(inputs)
    embedding_weight = torch.randn_like(embedding)
    dynamic_weight = torch.randn_like(detail_dynamic)
    loss = (embedding * embedding_weight).mean() + (
        detail_dynamic * dynamic_weight
    ).mean()
    loss.backward()
    ...
    expected_nonzero = {
        ...
        "dynamic_tokens": encoder.temporal_residual_projection[0].weight.grad,
...
def test_spatial_v2_warm_start_consumes_every_source_key_exactly():
    ...
    new_keys = sorted(set(dynamic_state) - set(source_state))
    assert report == {
