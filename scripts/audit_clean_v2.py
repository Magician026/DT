...
def main():
 p=argparse.ArgumentParser();p.add_argument('--clean',required=True);p.add_argument('--act',required=True);p.add_argument('--out',required=True);a=p.parse_args()
 ...
 sums={k:[0,0.,0.] for k in ('depth','marker')}; rows=[]
 for name in manifest['train']+manifest['val']:
  with h5py.File(root/name) as f:
   row={'file':name,'sensors':{}}
   for side in ('left','right'):
    s=f[f'tactile/{side}_gsmini'];n=len(s['rgb_marker']);m=s['marker'][::10];d=s['depth'][::10]
    if not np.isfinite(m).all() or not np.isfinite(d).all():raise ValueError(name)
    assert m.shape[1:]==(2,1200,2);assert d.shape[1:]==(240,320)
    assert np.allclose(m[:,0],m[0,0],atol=1e-5),f'nonstationary marker reference {name}'
    image=decode(s['rgb_marker'][0]);assert image.shape==(240,320,3)
    delta=m[:,1]-m[:,0]
    row['sensors'][side]={'frames':n,'depth_range':[float(d.min()),float(d.max())],'marker_displacement_range':[float(delta.min()),float(delta.max())],'image_shape':list(image.shape)}
    if name in manifest['train']:
     for key,v in [('depth',d),('marker',delta)]:
