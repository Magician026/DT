...
def aggregate(run_dirs,expected):
 ...
 for directory in run_dirs:
  ...
  contexts.append({**completion,'official_env_hashes':manifest['official_env_hashes'],'evaluation_command':f"{completion['task']} {manifest.get('task_config', 'demo')} ACT/deploy --total_num {len(requested)} --start_seed {requested[0]}",'episode_action_limit':{'insert_HDMI':600,'insert_tube':300,'pull_out_key':300}[completion['task']]})
